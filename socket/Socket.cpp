...
int Socket::recv(char *buf, int len) { return ::recv(m_sockfd, buf, len, 0); }
...
bool Socket::set_send_buffer(int size) {
  int buff_size = size;
  if (setsockopt(m_sockfd, SOL_SOCKET, SO_SNDBUF, &buff_size,
                 sizeof(buff_size)) < 0) {
    error("socket set send buffer error: errno=%d errstr=%s", errno,
          strerror(errno));
    return false;
  }
  return true;
...
bool Socket::set_recv_buffer(int size) {
  int buff_size = size;
  if (setsockopt(m_sockfd, SOL_SOCKET, SO_RCVBUF, &buff_size,
                 sizeof(buff_size)) < 0) {
    error("socket set recv buffer error: errno=%d errstr=%s", errno,
          strerror(errno));
    return false;
  }
  return true;
...
bool Socket::set_linger(bool active, int seconds) {
  ...
  if (setsockopt(m_sockfd, SOL_SOCKET, SO_LINGER, &l, sizeof(l)) < 0) {
    error("socket set sock linger error: errno=%d errstr=%s", errno,
          strerror(errno));
    return false;
  }
  return true;
...
bool Socket::set_keep_alive() {
  ...
  if (setsockopt(m_sockfd, SOL_SOCKET, SO_KEEPALIVE, &flag, sizeof(flag)) < 0) {
    error("socket set sock keep alive error: errno=%d errstr=%s", errno,
          strerror(errno));
    return false;
  }
  return true;
...
bool Socket::set_reuse_addr() {
  ...
  if (setsockopt(m_sockfd, SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(flag)) < 0) {
    error("socket set sock reuser addr error: errno=%s errstr=%s", errno,
          strerror(errno));
    return false;
  }
  return true;
...
bool Socket::set_reuse_port() {
  ...
  if (setsockopt(m_sockfd, SOL_SOCKET, SO_REUSEPORT, &flag, sizeof(flag)) < 0) {
    error("socket set sock reuser port error: errno=%d errstr=%s", errno,
          strerror(errno));
    return false;
  }
  return true;
